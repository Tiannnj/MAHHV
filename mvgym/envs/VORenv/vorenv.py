...
class vorenv(gym.Env):
    ...
    def __init__(self, grid_shape=(15, 15), n_agents=6, n_opponents=5, init_health=3, full_observable=False,
        ...
        self.action_space = MultiAgentActionSpace(
            [spaces.Discrete(12) for _ in range(self.n_o_agents)] + [spaces.Discrete(12) for _ in range(self.n_r_agents)])
#        print('self.action_space', self.action_space)
...
#        print('self.v_info',self.v_info)
        ...
        self.ou_action_space = MultiAgentActionSpace([spaces.Discrete(6 + 6) for _ in range(self.n_o_agents)])
        ...
        self.n_r_agents = 4
        self.m_location = np.array([[100, 0, 50], [200, 0, 50], [300, 0, 50], [400, 0, 50]])
        ...
        self.r_info = {_: np.concatenate((self.m_location[_], self.r_v[_],  self.r_assign[_])) for _ in range(self.n_r_agents)}
        self.ru_action_space = MultiAgentActionSpace([spaces.Discrete(6 + 6) for _ in range(self.n_r_agents)])
...
#        print('self.Mahhv_get_agent_obs()', self.Mahhv_get_agent_obs())
    ...
    def render(self, mode='human'):
        ...
        for agent_i in range(self.n_agents):
            if self.agent_health[agent_i] > 0:
                ...
                write_cell_text(img, text=str( self.agent_health[agent_i]), pos=self.agent_pos[agent_i], cell_size=CELL_SIZE,
...
#            print('agent_i, action', agent_i, action)
            if self.agent_health[agent_i] > 0:
...
#            print('o_agent_num, o_action', o_agent_num, o_action)
            band_o2v = 5
            band_o2r = 1
            # get the observation for OU
            o_pre_state = self.Mahhv_get_agent_obs()[o_agent_num]
            num_o_receive = o_pre_state[-4:-2]
...
#                    print('r_receive', r_receive)
                    ...
                    rate_o2r = band_even_r * math.log2(1 + SNR)
#                    print('rate_o2r', rate_o2r)
                    # delay from o to r
                    if rate_o2r > 0:
                        delay_o2r = (self.o_v[o_agent_num][3 + v * 6]) / rate_o2r
                    else:
                        delay_o2r = 0
...
#                    print('delay_v2o', delay_v2o, delay_o2r)
                    ...
                    if rate_o2r > 0:
                        ou_remain_ddl[o_agent_num][v] = np.array(list(self.v_info.values()))[v + o_agent_num * 4, 5] - ou_total_delay
                        # Generate new vehicle task information
                        self.o_v_new[o_agent_num][v * 6 + 5] = np.array(list(self.v_info.values()))[v + o_agent_num * 4, 5] - ou_total_delay
...
#            print('num_o_receive[o_agent_num]',num_o_receive[o_agent_num])
        ...
        # arrary to save the ddl for each ru
        r_ddl = [0, 0, 0, 0]
        ...
        for r_agent_num, r_action in enumerate(ru_action):
            ...
            for v in range (0, 6):
                r_pre_state[3 + v * 3 : 3 + v * 3 + 1] = np.multiply(r_pre_state[3 + v * 3 : 3 + v * 3 + 1], r_receive[r_agent_num][v])
            num_r_rtr = r_pre_state[21:23]
            # communication channel setting
            for m in range(0, 2):
                for v in range (0, 6):
                    # handle the task received by OU 1 assigned to RU 0 or RU 1
                    if r_receive[r_agent_num][v] == 1:
                        # distance between RU r and MeNB m
                        ...
                        if r_action[v] == 0:
                            num_r_rtr[0] = num_r_rtr[0] + 1
                        if r_action[v] == 1:
                            num_r_rtr[1] = num_r_rtr[1] + 1
                        # calculate the retransmit delay
                        delay_r2m = (np.array(list(self.v_info.values()))[v, 3])/ rate_r2m
                        # calculate the compute delay
                        delay_m_compute = (np.array(list(self.v_info.values()))[v, 4]) / self.m_cpu[int(r_agent_num * 2 + r_action[v])]
                        ...
                        self.r_m_new[r_o][v * 5 + 5] = self.o_v_new[r_o][v * 5 + 5]
                        r_ddl[r_agent_num] = r_ddl[r_agent_num] + self.r_m_new[r_o][v * 5 + 5]
                # The fairness in the RU observation state has to be changed
            self.r_assign[r_agent_num] = num_r_rtr
        ...
        for agent_o in range(0, self.n_o_agents):
            ...
            o_delay = sum(ou_delay[agent_o])
#            print('o_delay', self.o_receive, o_delay, f_o_rec, f_o_tra)
...
#            print('rewards_ou', rewards_ou)
        ...
        new_state[0 : o_agent_num, -4: -2] = num_o_receive[:]
        for xx in range (0, o_agent_num):
            new_state[0 : xx, -2: ] = self.o_tra[xx]
        for yy in range (0, r_agent_num):
            new_state[2 : 2 + yy, -2: ] = self.r_assign[yy]  
    
        if (self._step_count >= self._max_steps) :
            self._agent_dones = [True for _ in range(self.n_agents)]

        return new_state, rewards, self._agent_dones
